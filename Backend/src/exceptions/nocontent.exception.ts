...
import {
  ...
  API_STATUS_CODES, API_STATUS_MESSAGES,
  APPLICATION_ERROR_CODES,
} from '../enum/errorsCode'
import { N } from '../helpers'


export class CreateException extends HttpException {
  constructor(error: string | APPLICATION_ERROR_CODES, extended?: Record<any, any>, error_status_code?: number) {
    const processError = (code: any) => {
      let status_code = 500

      const message = API_ERROR_CODES_MESSAGES[code] || API_ERROR_CODES_MESSAGES[API_ERROR_CODES.UNEXPECTED_ERROR]
