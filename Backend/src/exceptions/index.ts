...
export class HttpExceptionFilter implements ExceptionFilter {
    ...
    if (typeof exception?.response?.message === 'object' && exception?.response?.message?.length) {
      result.errors = []
      exception?.response?.message?.forEach(item => {
        result.errors.push({ code: exception?.response?.status_code || API_ERROR_CODES.INTERNAL_ERROR, message: item })
      })
    }
